    ...
    <Form {...form}>
      <div className="mx-auto max-w-4xl space-y-8 py-6 md:py-10">
        <div className="flex items-center justify-between border-b border-zinc-200 pb-6">
          <div>
            <p className="text-sm text-zinc-500">{locale === "ar" ? "معلومات عضو الفريق" : "Team member details"}</p>
            <Text variant="h2" className="mt-1">{values?.name?.[locale] || t("author")}</Text>
          </div>
          {values?.id && (
        ...
        <div className="grid gap-8 md:grid-cols-[220px_1fr] md:items-start">
          <FormUpload className="aspect-[4/5] w-full overflow-hidden border border-zinc-200 bg-zinc-50" form={form} label={t("image")} name="image" />
        ...
        <Button isLoading={isPending} onClick={form.handleSubmit(onSubmit)} type="submit" className="border-[#b9784d] bg-[#b9784d] text-white hover:bg-[#9c603a] hover:text-white">
          {t("update")}
        </Button>
      </div>
    </Form>
